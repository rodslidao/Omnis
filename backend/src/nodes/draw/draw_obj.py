...
class DrawOBJ(dimensional_data):
    ...
    def drawVertices(self):
        for n in self.edges.values():
            arrowedLine(
                self.image, self.edges["A"], n, (255, 0, 0), 1, line_type=LINE_AA
            )
    ...
    def drawRectSize(self):
        for _, l in enumerate(["AB", "AC", "AD"]):
            putText(
                self.image,
                f"{l}: {round(self.get(l),2)}",
                (self.edges["C"][0], self.edges["B"][1] + 25 + int(25 * _)),
                FONT_HERSHEY_SIMPLEX,
                0.5,
                (255, 255, 255),
                1,
    ...
    def drawAngles(self):
        ...
        putText(
            self.image,
            f"{round(self.angle, 2)}",
            (self.center["X"], self.center["Y"] - 25),
            FONT_HERSHEY_SIMPLEX,
            0.3,
            (255, 255, 255),
            1,
