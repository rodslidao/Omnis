    ...
    >
      ...
      <div v-if="device.type === 'slider'" class="pr-4 pt-4">
        ...
        <div class="d-flex">
          <v-slider
            class="slider"
            v-model="device.value.selected"
            :min="device.value.min"
            :max="device.value.max"
            v-haptic
          >
          ...
          <div>
            <v-text-field
              ...
              type="number"
              v-on:keyup.enter="
                (variableList[index].manualMode = false), sendDistance(index)
              "
              @blur="
                (variableList[index].manualMode = false), sendDistance(index)
              "
...
export default {
  ...
  methods: {
    increment(index) {
      this.outputDevices[index].value.selected += 1;
    },
